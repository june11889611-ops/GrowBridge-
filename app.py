...
--------------------------------------------------------------------------
컨셉: 대학원(학술) ↔ 산업의 연결을 매주 번역해 주는 에이전트.
...
def find_recent_papers(keywords: str, days: int = 7, max_results: int = 6,
    ...
    try:
        ...
        if not picked:  # 최근 days일 내 결과가 없으면 최신 논문으로 대체
            ...
            note = f"(최근 {days}일 내 신규 논문이 없어 가장 최신 논문으로 대체했습니다.)\n"
        if not picked:
...
def set_research_focus(field: str, keywords: str) -> str:
    """사용자의 연구 분야와 핵심 키워드를 저장한다(주간 추천의 기준)."""
    st.session_state.research_focus = {"field": field, "keywords": keywords}
    return f"연구 관심사 저장 완료 — 분야: {field} / 키워드: {keywords}"
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "사용자의 연구 분야와 핵심 키워드를 저장한다. 사용자가 자기 관심 분야를 처음 밝힐 때 사용한다.",
            "parameters": {
                "type": "object",
                "properties": {
                    "field": {"type": "string", "description": "연구/관심 분야"},
                    "keywords": {"type": "string", "description": "핵심 키워드(쉼표 구분)"},
                },
...
SYSTEM_PROMPT = (
    ...
    "임무: 사용자의 연구·관심 분야에 맞는 최신 논문을 찾아 주고, 각 논문을 '학술'에서 그치지 않고 "
    "'산업으로의 연결'까지 번역해 주는 것이다.\n"
    "원칙:\n"
    "1) 사용자가 분야/키워드를 밝히면 set_research_focus로 저장한다.\n"
    "2) 최신·추천 논문 요청 시 find_recent_papers를 호출한다. 키워드는 영문으로 변환해 검색한다.\n"
    ...
    "6) 과장 없이, 실무자가 바로 쓸 수 있게 구체적으로 답한다. 논문 내용을 지어내지 말고 "
    "검색된 초록에 근거한다."
)
...
# ----------------------------------------------------------------------------
st.title("🌉 GrowBridge")
st.markdown("##### 최신 논문을 찾아 **학술→산업 연결**까지 번역해 주는 주간 큐레이터 에이전트")

if not st.session_state.display:
    st.info(
        "관심 분야를 말하고 추천을 받아보세요 👇\n\n"
        "- \"내 분야는 자연어처리(RAG, LLM 평가)야. 이번 주 새 논문 추천하고 산업 연결까지 정리해줘\"\n"
        "- \"강화학습 최신 논문 5편 요약하고, 어떤 산업에 쓰일 수 있는지 알려줘\"\n"
